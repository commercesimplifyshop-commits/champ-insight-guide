...
const Index = ({ initialSolo = false }: IndexProps) => {
  ...
  // /?role=top&ally=Yasuo&enemy=Zed (role optional — the /campeoes wiki CTA
  // only sends ally, since a champion isn't tied to one role) — resolves the
  // ids against the same champion search endpoint ChampionPicker/ChampionSheet
  // already use.
  useEffect(() => {
    ...
    if (!prefillAlly) return;
    if (prefillRole && validRoles.includes(prefillRole as Role)) setRole(prefillRole as Role);
